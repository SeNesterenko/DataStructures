...
namespace DataStructures
{
     ...
     where T : IComparable<T>
    {
        ...
        {
            var result = _heap[0]; // Gets the first element because it has the maximum value
            _heap.Set(0, _heap[^1]); // Sets a minimal element to the first index
        ...
        private void ShiftUp(int index) // Sorts elements by parent 
        {
            ...
            if (_heap[index]!.CompareTo(_heap[parentIndex]) <= 0) return; // If a value of index < a value of parentIndex just returns
        ...
        private void ShiftDown(int index) // Sorts elements by child
        {
            ...
            if (rightChildIndex < _heap.Count &&
                _heap[leftChildIndex]!.CompareTo(_heap[rightChildIndex]) < 0) // If the leftChild is less than the rightChild
            {
                maxChildIndex = rightChildIndex; // The rightChild is larger
            }

            if (_heap[index]!.CompareTo(_heap[maxChildIndex]) > 0) // If the maxChildIndex > index just returns
            {
                return;
            }
        ...
        private class HeapEnumerator : IEnumerator
        {
            ...
            public object Current => _heap[_index];
        }
    }
}